...
from hyperopt import hp, tpe, fmin, Trials, STATUS_OK, space_eval, STATUS_FAIL, rand, partial

from model_super import build_and_train, build_model, MODEL_NAME

SAVE_RESULTS = "results_mod_super_v4.pkl" # save trials of optimization
SAVE_BEST_PLOT = "model_super_best_v4" # save best NN graph
...
space = {
    ...
    'embedding': hp.choice('embedding', [True, False]),
...
def run_a_trial():
    ...
    best = fmin(
        optimize_model,
        space,
        algo=partial(tpe.suggest, n_startup_jobs=160), #
        trials=trials,
        max_evals=max_evals
    )
